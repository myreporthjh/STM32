...
void pc_command_processing(void)
{
	if(newline_detect_flag) // comport master로부터 완전한 문장이 들어오면 (\n을 만나면)
	{
		...
		if(!strncmp(rx_buff, "setrtc", strlen("setrtc"))) //setrtc
		{
			set_rtc(rx_buff);
			return;
		}
...
//		}
//		// 과제2번
//		if(!strncmp(rx_buff, "dht11on", strlen("dht11on"))) //온습도출력
//		{
//			toggle_DHT=1;
//			return;
//		}
//		if(!strncmp(rx_buff, "dht11off", strlen("dht11off"))) //온습도출력종료
//		{
//			toggle_DHT=0;
//			return;
//		}
//		if(!strncmp(rx_buff, "ultra_on", strlen("ultra_on"))) //거리정보출력
//		{
//			toggle_sonic=1;
//			return;
//		}
//		if(!strncmp(rx_buff, "ultra_off", strlen("ultra_off"))) //거리정보출력종료
//		{
//			toggle_sonic=0;
//			return;
//		}
